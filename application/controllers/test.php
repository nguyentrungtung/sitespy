...
class Test extends CI_Controller
{
	...
	public function check_proxy_ip(){
		...
		if($httpcode!='200'){
			echo "{$proxy}: Dead - HTTP Code : {$httpcode}<br>";
		 }
		 
		else
			echo "{$proxy}: Live  - HTTP Code : {$httpcode}<br>"; 
  ...
  public function sk_test(){

	//$response = $this->web_common_report->dns_information("facebook.com");
	//$response = $this->web_common_report->dns_information('xeroneit.net');
	/*
	$response1 = $this->web_common_report->get_header_response('facebook.com');
	$response = explode(PHP_EOL, $response1);
	...
	foreach($response as $single_response) :
		...
		if($semicolon_position !== FALSE) :
			...
			$str .= "<tr><td>" . $title . "</td><td>" . $value . "</td></tr>";
		else :
			if($counted != 0 && $single_response != '') :
				$str .= "</table><br/>";
			endif;
				
			if($single_response != '') :
				$str .= "<table border='1'><caption>" . $single_response . "</caption>";
				$str .= "<tr><th>Field</th><th>Value</th></tr>";
				$counted = 1;
			endif;	
		endif;	
	endforeach;	
